...
def button_init(DIMENSIONS,menu_screen):
    ...
    menu_screen.blit(QUIT_TEXT,(DIMENSIONS[0]/2.0 -244/2, DIMENSIONS[1] / 2.0 +25.0 ))
    pygame.draw.rect(menu_screen,BUTTON_COLOR,BUTTON_PLAY)
    menu_screen.blit(PLAY_TEXT,(DIMENSIONS[0]/2.0 -244/4, DIMENSIONS[1] / 3.0 + DIMENSIONS[1]/3 +25))

def button_select(menu_screen,DIMENSIONS):
    global down1, down2,quit_selected,play_selected,QUIT_TEXT,PLAY_TEXT
    keys=pygame.key.get_pressed()
    if (keys[pygame.K_DOWN] or down1) and not down2:
        ...
        QUIT_TEXT = QUIT_FONT.render("QUIT GAME", True,(0,quit_selected,0))
        PLAY_TEXT = PLAY_FONT.render("PLAY",False,(0,play_selected,0))
    if (keys[pygame.K_DOWN] and down1)or down2:
        down1=False
        down2=True
        play_selected=200 
        quit_selected=0
...
def title_animation(menu_screen,DIMENSIONS):
    ...
    if 480>T_anim_timing >= 450 or 542>T_anim_timing>=520 :
        ...
        menu_screen.blit(TITLE_TEXT, (DIMENSIONS[0]/4,DIMENSIONS[1]/18))
    else:
        TITLE_TEXT = TITLE_FONT.render("Furious Lardinus", True,(0,0,0))
