...
def draw_tile_map() -> None:
			...
			for tile_index_y in range(start_tile_index, end_tile_index + ray_direction_sign, ray_direction_sign):
				...
				if 0 <= tile_index_x < Game.tile_map_size[X]:
					...
					if tile is not None:
						distance = ray_length
						tile = tile
						texture_offset = (ray_position_x % Game.tile_size[X]) / Game.tile_size[X]
						break
	...
	for _ in range(Settings.RAYS_NUMBER):
		...
		if (0 < distance_to_vertical) and (distance_to_vertical < distance_to_horizontal or distance_to_horizontal <= 0):
			draw_column(ray_relative_angle, distance_to_vertical, vertical_tile, int(Game.tile_size.y), vertical_texture_offset)
		elif (0 < distance_to_horizontal):
			draw_column(ray_relative_angle, distance_to_horizontal, horizontal_tile, int(Game.tile_size.x), horizontal_texture_offset)
...
def get_sprites() -> None:
	for game_object in Game.object_container:
		...
		vector_to_object: pygame.Vector2 = object_position - position
		distance: float = vector_to_object.length()
		object_relative_angle: float = vector_to_object.angle_to() - rotation
		if distance == 0:
			continue
		if (Settings.half_fov_h < abs(object_relative_angle)) and (object_radius > math.cos(abs(object_relative_angle) - Settings.half_fov_h) * distance):
			continue
