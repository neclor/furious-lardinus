	...
	for ray in range(RAYS_NUMBER):
		...
		for length in range(RAY_LENGTH):
			...
			if (x // WALL_SIZE, y // WALL_SIZE) in level_map:
				...
				projection_height: float = WINDOW_SIZE[1]
				if distance != 0:
					projection_height = WALL_HEIGHT / (2 * distance * math.tan(FOV_V / 2)) * WINDOW_SIZE[1]

					print(WALL_HEIGHT / (2 * distance * math.tan(FOV_V / 2)))
				screen_x = ray * RAY_WIDTH
				screen_y = (WINDOW_SIZE[1] - projection_height) / 2

				screen_rect = (ray * RAY_WIDTH, screen_y, RAY_WIDTH, projection_height)
